...
public class ControladorAvaliacoes {
    ...
    public Boolean adicionarAvaliacao(AvaliacaoRU avaliacao){
        ...
        if( !(avaliacao.getTurno() == "almoço" || avaliacao.getTurno() == "jantar") ) {
            return false;
        }
        if( !(avaliacao.getProteina() == "animal" || avaliacao.getProteina() == "vagetal") ){
            return false;
        }
    ...
    public String getTodosComentarios(){
        ...
        while(i < cursor) {
            if (avaliacoes[i].getComentario() != "") {
                resultado += avaliacoes[i].getData()+ " - "+ avaliacoes[i].getTurno() + " - " + avaliacoes[i].getProteina()+ " - "+ avaliacoes[i].getNota() + " - "+ avaliacoes[i].getComentario()+ "\n";
            }
            i++;
        }
        return resultado;
